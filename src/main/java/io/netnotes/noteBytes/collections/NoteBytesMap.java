package io.netnotes.noteBytes.collections;
...
public class NoteBytesMap{
    ...
    public NoteBytesReadOnly getReadOnly(String key) {
        NoteBytes value = m_pairs.get(new NoteBytes(key));

        return value != null ? value.readOnly() : null;
    }

    public String getAsString(NoteBytes key, String defaultString){
        NoteBytes noteBytes = get(key);
        return noteBytes != null ? noteBytes.getAsString() : defaultString;
    }

    public boolean getAsBoolean(NoteBytes key, boolean defaultBoolean){
        NoteBytes noteBytes = get(key);
        return noteBytes != null ? noteBytes.getAsBoolean() : defaultBoolean;
    }

    public long getAsLong(NoteBytes key, long defaultLong){
        NoteBytes noteBytes = get(key);
    ...
    public double getAsDouble(NoteBytes key, double defaultDouble){
        NoteBytes noteBytes = get(key);
        return noteBytes != null ? noteBytes.getAsDouble() : defaultDouble;
    }
    ...
    public void put(NoteBytes key, double d){
        m_pairs.put(key, new NoteBytes(d));
    }
