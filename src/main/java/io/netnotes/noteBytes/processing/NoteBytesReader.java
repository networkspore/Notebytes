...
public class NoteBytesReader implements AutoCloseable{
    ...
    public NoteBytesReader(InputStream is, int bufferSize){
        m_in = is;
    ...
    public byte[] readNextBytes(int size) throws IOException{
        ...
        int bufferSize = size < this.bufferSize ? size : this.bufferSize;
    ...
    public int readNextBytes(byte[] buf, int offset, int length) throws IOException{
        ...
        int bufferSize = length < this.bufferSize ? length : this.bufferSize;
        byte[] buffer = new byte[bufferSize];
        int readLength = 0;
